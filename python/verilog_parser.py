...
def parse_verilog_file(fname):
    ...
    '''
    with open(fname, 'rt') as verifile:
        text = verifile.read()
...
class VerilogExtractor():
    ...
    def extract_objects(self, fname, type_filter=None):
        ...
        if fname in self.object_cache:
            ...
            with io.open(fname, 'rt', encoding='utf-8') as verifile:
                text = verifile.read()
    ...
    @classmethod
    def extract_objects_from_source(cls, text, type_filter=None):
